...
class CacheService {
  ...
  Future<void> cacheProducts(List<dynamic> products,
    ...
    final cacheKey =
        '${shopId ?? category}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<List<dynamic>?> getCachedProducts(
      {String? shopId, String? category}) async {
  ...
  Future<void> cacheOffers(List<dynamic> offers, {String? shopId}) async {
    final cacheKey =
        '${shopId ?? 'all'}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> clearCategoryCache(String category) async {
    ...
    for (final key in shopKeys) {
      await _shopsCache.delete(key);
    }
    for (final key in productKeys) {
      await _productsCache.delete(key);
    }
  }
